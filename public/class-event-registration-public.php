 ...
 * @subpackage wp-event-registration/public
 */
...
class Event_Registration_Public {
  ...
  /**
	 ...
	 * @since     1.0.0
	 */ 
	public function send_email( $to, $to_email, $from, $from_email, $subject, $message, $attachments='', $to2=true ){
    ...
    public function wpecr_registration( $atts ) {
    	...
    	if( isset( $_POST['action'] ) && $_POST['action'] =='order_send' ) {
				...
				if ( $page_message= $this->wpecr_registration_send() ) {					
					...
					$data .='<p>'. __( 'We sent you an email with your registration details.', 'wpecr' ) .'</p>';
				}else{
					//NO email
					$data ='<p class="wpecr_error">'. __( 'Sorry, your registration was not sent. Please get in contact with us.', 'wpecr' ) .'</p>';
				}
    ...
    public function wpecr_registration_send() {
		...
		$subject = sprintf( __( 'Registration: %1$s', 'wpecr' ), $_POST['wpecr']['event'] ).' '.$_POST['wpecr']['date'].' ';
		$subject .= __( '(automatic reply)', 'wpecr' );

		$message = $page_message = sprintf( __( 'Thank you %1$s, <br/><br/> your registration was successfull.', 'wpecr' ), $to );
		...
		$message .= '<br/><br/>'. $_POST['wpecr']['event'].' | '.$_POST['wpecr']['date'].' | '.$_POST['wpecr']['price'];
		...
		if ( !empty( $options['showbank'] ) ) {
		    ...
		    $message .= '<br/><br/>'.__( 'Please transfer the invoice total to:', 'wpecr' );
			$message .= '<br/> '.$options['bank'];
			$message .= '<br/>'.__( 'Remittee', 'wpecr' ).' '.$options['remittee']; 
			$message .= '<br/>'.__( 'IBAN', 'wpecr' ).' '.$options['iban']; 
			$message .= '<br/>'.__( 'Swift / BIC Code', 'wpecr' ).' '.$options['bic'];  
			$message .= '<br/>'.__( 'Reason for payment', 'wpecr' ).' '.$_POST['wpecr']['event'];	
			$message .= '<br/>'.__( 'Amount', 'wpecr' ).' '.$_POST['wpecr']['price'];	
			$message .= '<br/><br/>'.__( 'Thank you!', 'wpecr' );
		}
		// footer from ADMIN options
		$message .= '<br/><br/>';
		//echo 'MAIL:'.
		$message .= stripslashes( $options['email_txt'] );
    ...
    public function format_array( $related_post, $mode = 1 ) {
        ...
        $var_array = array(
        		array( 'name' => __( 'Please select', 'wpysm' ), 'value' => -1 )
        	);
